...
class UR5eEnv:
    ...
    # action_bound0-action_bound4 对应 theta1-theta5 的 action 的每step的转动动作范围
    action_bound0 = [-0.0189, 0.0189]
    ...
    def reset(self):
        ...
        reset_state = np.concatenate((self.arm_info['theta'], self.target_info['target_loc'])) # 将两个ndarray拼成一个
        return reset_state
    ...
    def step(self, action, target_position, goal=False,  end_point=False, point0=False, point1 = False, point2 = False):
        ...
        :return:[状态[theta1, theta2, theta3, theta4, theta5,theta6], 碰到目标的奖励reward, 是否已经完成done]
        """
        self.episode_step += 1
        done = False
        r = 0
