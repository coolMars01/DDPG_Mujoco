...
class UR5eSimulator:
    def __init__(self, visualize=True, save_table=True, plot_joint=True, test_mode=False):
        ...
        self.agent = PPO(self.state_dim, self.action_dim, self.max_action)
        ...
        # 画图及数据保存相关变量
        self.plot_timestep = 500        # 每 plot_timestep 画一个episode奖励变化曲线
    ...
    def run(self):
        ...
        while episode_counter < self.max_episode:
            ...
            while not done:  # 一次while就是一个episode
                ...
                # DDPG为off-policy需要经验回放池，PPO为on-policy禁止使用旧数据
                transition_dict['states'].append(self.state)        # 经验回放：形成轨迹数据，为后续策略更新做准备
            ...
            self.agent.update(transition_dict) # 使用整个episode的完整轨迹数据进行批量更新
...
if __name__ == '__main__':
    sim = UR5eSimulator(visualize=False, save_table=False, plot_joint=False, test_mode=False)
    sim.run()
